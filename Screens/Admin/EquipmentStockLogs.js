...
import ListItem from "./EquipmentStockLogsItems";
...
const EquipmentStockLogs = (props) => {
  const [equipmentLogList, setEquipmentLogList] = useState();
  const [EquipmentLogFilter, setEquipmentLogFilter] = useState();
  ...
  const searchCalendarLog = (text) => {
    if (text === "") {
      setEquipmentLogFilter(equipmentLogList);
    }
    setEquipmentLogFilter(
      equipmentLogList.filter((i) =>
  ...
  const onRefresh = useCallback(() => {
    setRefreshing(true);
    setTimeout(() => {
      axios.get(`${baseURL}equipments/stockHistory/all`).then((res) => {
        // Sort the data by createdAt field in descending order
        const sortedData = res.data.sort((a, b) => {
          return new Date(b.createdAt) - new Date(a.createdAt);
        });
        setEquipmentLogList(sortedData);
        setEquipmentLogFilter(sortedData);
        setLoading(false);
      });
      setRefreshing(false);
    }, 2000);
  ...
  const filterByStatus = (status) => {
    ...
    if (status === "All") {
      ...
      const filteredLogs = equipmentLogList.filter(
        (item) => item.status === status
      );
      setEquipmentLogFilter(filteredLogs);
    }
  };
...
export default EquipmentStockLogs;
